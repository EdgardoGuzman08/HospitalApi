...
const controladorUsuariosPacientes = require('../controladores/controladorUsuarioPacientes');
const { body, query } = require('express-validator');

const rutas = Router();

rutas.get('/', controladorUsuariosPacientes.Inicio);
rutas.get('/listar', controladorUsuariosPacientes.Listar);
...
controladorUsuariosPacientes.Guardar);
...
body('contrasena').isLength({min:3, max: 50}).withMessage('La contrasena del usuario debe tener mas de 3 caracteres'),
body('correo').isLength({min:3, max: 50}).withMessage('El correo del usuario debe tener mas de 3 caracteres'),
controladorUsuariosPacientes.Editar);

rutas.delete('/eliminar', 
query('id').isInt().withMessage('Solo se permiten valores enteros'),
controladorUsuariosPacientes.Eliminar);
